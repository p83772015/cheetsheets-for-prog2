...
#+begin_src C++
  ...
  int *
  find_lo_hi_in_file(fstream *file_obj)
  {
    int line;
    int *hi_lo_tuple = (int *)malloc(2 * sizeof(int));
    int highest;
    ,*file_obj >> highest;
    int lowest = highest;
    while (*file_obj >> line) {
    ...
    hi_lo_tuple[0] = lowest;
    hi_lo_tuple[1] = highest;
    return hi_lo_tuple;
  }
  ...
  main ()
  {
    ...
    // Compute and print result
    lo_hi = find_lo_hi_in_file(&file_obj);
    cout << "Smallest integer: " << lo_hi[0] << '\n'
         << "Largest integer: " << lo_hi[1] << '\n';
